...
impl<T> Bounds2<T> where T: Num + Copy {
...
pub struct Bounds2Iterator<'a, T> where T: Num {
    pub p: Vector2<T>,
    pub bounds: &'a Bounds2<T>
}

impl<'a, T> Bounds2Iterator<'a, T> where T: Num + Copy {
...
impl<'a, T> Bounds2<T> where T: Num + Copy {
...
mod tests {
    ...
    fn test_iterate() {
        ...
        let reference = vec![
            ...
            Vector2 { x: 0, y: 2 }, Vector2 { x: 1, y: 2 },
            Vector2 { x: 0, y: 3 }, Vector2 { x: 1, y: 3 }];
