...
impl PinHoleCamera {
    pub fn new(resolution: &Vector2u, fov: f64, near: f64, far: f64, cam_to_world: Matrix4) -> Self {
        ...
        let (screen_p_min_x, screen_p_max_x, screen_p_min_y, screen_p_max_y) = if image_aspect_ratio > 1.0 {
...
impl Camera for PinHoleCamera {
    fn get_ray(&self, pixel_x: f64, pixel_y: f64) -> Ray {
        ...
        Ray::new(
            &self.cam_to_world.transform_point(&Vector3f::new(0.0, 0.0, 0.0)),
            &self.cam_to_world.transform_direction(&camera_vector),
        )
    }
}
