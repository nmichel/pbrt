...
use pbrt::geom::vector2::{Vector2, Vector2f, Vector2u};
use pbrt::geom::vector3::Vector3f;
use pbrt::integrators::integrator::Integrator;
use pbrt::integrators::PathIntegrator;
...
fn main() {
    ...
    let camera: Box<dyn Camera> = Box::new(PinHoleCamera::new(
        &resolution,
        fov,
        near,
        far,
    ...
    let integrator_ptr_transmuted =
        unsafe { mem::transmute::<&dyn Integrator, &'static dyn Integrator>(&integrator) };
    let scene_ptr_transmuted = unsafe { mem::transmute::<&Scene, &'static Scene>(&scene) };
    let camera_ptr_transmuted =
        unsafe { mem::transmute::<&dyn Camera, &'static dyn Camera>(&*camera) };
